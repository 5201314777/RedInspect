...
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QLabel, QComboBox, QApplication
...
#         self.index = index  # 存储窗口的索引或标识符
#         self.setStyleSheet("QWidget {background-color:white;}")
#
#     def mousePressEvent(self, event):
#         print(f"Clicked on video widget at index: {self.index}")
#
# class VideoView(QtWidgets.QWidget):
#     """
#     视频视图类，用于显示视频内容。
...
#         self.video_widgets = []
#
#         # 假设 ToolBar 和 utils 已经正确实现并导入
#         self.video_operation_bar = ToolBar("视频操作")  # 使用自定义的 ToolBar 或 PyQt5 的 QToolBar
...
#         # 创建一个QWidgetAction来包含窗口数选择的组合框
#         selectWidNum = QtWidgets.QWidgetAction(self)
#         widNumWidget = QWidget()
#         selectWidNum.setDefaultWidget(widNumWidget)
#
#         widNumLayout = QHBoxLayout(widNumWidget)
#         selectWidNumLabel = QLabel("窗口数:")
#         selectWidNumLabel.setAlignment(Qt.AlignCenter)
#         widNumLayout.addWidget(selectWidNumLabel)
#
#         self.selectWidNumComboBox = QComboBox()
#         self.widNums = ['1', '4', '9', '16', '25', '36', '49', '64', '81']
#         self.selectWidNumComboBox.addItems(self.widNums)
#         self.selectWidNumComboBox.setCurrentIndex(0)
#         self.selectWidNumComboBox.currentIndexChanged.connect(self.updateVideoLayout)
#         widNumLayout.addWidget(self.selectWidNumComboBox)
#
#         startVideo = action("播放", self.startVideo)
#         record = action("录像", self.record)
#         grabbing = action("抓图", self.grabbing)
#         inforce = action("强制帧", self.inforce)
#
#         self.actions = utils.struct(
#             startVideo=startVideo,
#             record=record,
...
#             for j in range(n):
#                 # 使用自定义的 VideoWidget
#                 video_widget = VideoWidget(i * n + j)
#                 HLayout.addWidget(video_widget)
#                 self.video_widgets.append(video_widget)
#             self.video_layout.addWidget(container)